...
def start_scene():
    ...
    if CURRENT_FRAME < len(pool.Frames):
        ...
        if settings.get("uName") in scene.cartes.keys() and scene.start_etapes[settings.get("uName")] is not None:
            ...
            tools.log_teleco(("start scene", "{0}+{1}".format(scene.name, CURRENT_FRAME-CURRENT_SCENE_FRAME)), "scenario")
...
def stop_scene():
    stop_flag = fsm.Flag("SCENE_STOP", JTL=0, TTL=None)      # TODO : refactor this in an other place !
