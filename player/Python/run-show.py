...
import libs.oscack as OSC
from engine import fsm, threads
from scenario import parsing, pool
from scenario import manager

from libs import rtplib, subprocess32
from modules import video, audio


# def test_sound_omx(path, args, types, engine):
#     log.info("recv")
...
#
#
# def test_sound_aplay(path, args, types, engine):
#     log.info("recv")
#     t = rtplib.get_time()
#     log.info("test_sound current time {0}s {1}ns ".format(t[0], t[1]))
#     log.info("test_sound wanted  time {0}s {1}ns ".format(args[0], args[1]))
...
#
#
# def test_sound_pygame(path, args, types, engine):
#     log.info("recv")
#     t = rtplib.get_time()
#     log.info("test_sound current time {0}s {1}ns ".format(t[0], t[1]))
#     log.info("test_sound wanted  time {0}s {1}ns ".format(args[0], args[1]))
...
#     # END TIME CRITICAL
#
# def test_nosync_sound_omx(path, args, types, engine):
#     log.info("recv")
#     player = video.OMXVideoPlayer("test_sound.wav", hardware=False, audio=False)
#     player.start()
#     log.info("release !")
#
#
# def test_nosync_sound_aplay(path, args, types, engine):
#     log.info("recv")
#     path = os.path.join(settings.get("path", "media"), "test_sound.wav")
#     subprocess32.Popen(["/usr/bin/aplay", "-q", path])
#
#
# def test_nosync_sound_pygame(path, args, types, engine):
#     log.info("recv")
#     path = os.path.join(settings.get("path", "media"), "test_sound.wav")
...
try:
    threads.init()
    OSC.start_protocol()
    # OSC.DNCengine.add_method("/test/sync/omx", None, test_sound_omx)
    # OSC.DNCengine.add_method("/test/sync/aplay", None, test_sound_aplay)
    # OSC.DNCengine.add_method("/test/sync/pygame", None, test_sound_pygame)
    # OSC.DNCengine.add_method("/test/nosync/omx", None, test_nosync_sound_omx)
    # OSC.DNCengine.add_method("/test/nosync/aplay", None, test_nosync_sound_aplay)
    # OSC.DNCengine.add_method("/test/nosync/pygame", None, test_nosync_sound_pygame)
    parsing.parse_scenario(os.path.join(settings.get("path", "scenario"), "scenario_test.json"))
    ...
    while True:
        ...
        if cmd[0] == "info":
            ...
            log.info(OSC.DNCengine.networkmap)
