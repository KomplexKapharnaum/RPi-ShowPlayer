        ...
        try:
            while True and not self._stop.is_set():
                ...
                if cmd[0] == "pw":
                    ...
                    log_fnct("{:<20}: Poweroff".format("pw"))
                    log_fnct("{:<20}: ?".format("except"))
                    log_fnct("{:<20}: Access the FSM history tool".format("history"))
                    log_fnct("{:<20}: Access the FSM multiplex history tool".format("mhistory"))
                    log_fnct("{:<20}: Prompt the current scene".format("timeline"))
                    log_fnct("{:<20}: Prompt the scene number #1 parsed json data".format("scene (int)"))
                    log_fnct("{:<20}: Prompt the json timeline".format("timelinejson"))
                    log_fnct("{:<20}: Prompt the json scenario".format("scenariojson"))
                    log_fnct("{:<20}: Prompt the input #1 to the teleco".format("_log (str..)"))
                    log_fnct("{:<20}: Prompt the settings ".format("settings"))
                    log_fnct("{:<20}: Prompt MEDIA SYNC infos ".format("msync"))
                    log_fnct("{:<20}: Prompt SCENARIO SYNC infos ".format("ssync"))
                    log_fnct("{:<20}: Prompt REAL TIME PROTOCOL infos ".format("rtp"))
                    log_fnct("{:<20}: Prompt OSC infos ".format("osc"))
                    log_fnct("{:<20}: Prompt SCENARIO infos ".format("scenario"))
                    log_fnct("{:<20}: Prompt SCENARIO FSM infos ".format("sfsm"))
                    log_fnct("{:<20}: Prompt several infos ".format("info"))
                    log_fnct("{:<20}: Send test signal to all (#1==testall) or to group (#1==testgroup) ".format("signal (str)"))
                    log_fnct("{:<20}: Log #2 to the teleco on page number #1 ".format("teleco (int) (str..)"))
                    log_fnct("{:<20}: Log #2 on level #1 ".format("log (int) (str..)"))
                    log_fnct("{:<20}: Evaluate with python the experssion #1 ".format("eval (str..)"))
                    log_fnct("{:<20}: Prompt this help".format("help"))
                else:
