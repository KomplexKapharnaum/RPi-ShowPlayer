...
def server_sync(flag):
    ...
    OSC.DNCserver.ackServer.del_method("/rtp/pong", None)
    OSC.DNCserver.ackServer.add_method("/rtp/pong", None, catch_pong)
    network_scheduler.enter(settings.get("rtp", "timeout"), machine.append_flag,
    ...
    while not machine.current_state.stop.is_set():
        ...
        if sync != 1:
            ...
            break
    OSC.DNCserver.ackServer.del_method("/rtp/pong", None)
    machine.append_flag(flag_timeout_task_sync.get(TTL=0.5, JTL=1))
    #log.log("raw", "End server sync")


def client_sync(flag):
    #log.log("raw", "Start client sync")
    target = message.Address(flag.args["src"].get_hostname())
    ...
    def catch_ping(path, args, types, src):
        ...
        # END # TIME CRITICAL
    OSC.DNCserver.ackServer.del_method("/rtp/ping", None)
    OSC.DNCserver.ackServer.add_method("/rtp/ping", None, catch_ping)
    OSC.DNCserver.ackServer.del_method("/rtp/sync", None)
    OSC.DNCserver.ackServer.add_method("/rtp/sync", None, client_get_sync)
    network_scheduler.enter(settings.get("rtp", "timeout"), machine.append_flag,
                            flag_timeout_wait_sync.get(
                                TTL=settings.get("rtp", "timeout") * 1.5, JTL=4))
    machine.current_state.preemptible.set()
