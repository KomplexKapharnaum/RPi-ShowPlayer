...
class NoExtensionTestSuite(tests.TestSuite):
    def run(self, result):
        import simplejson
        simplejson._toggle_speedups(False)
        result = tests.TestSuite.run(self, result)
        simplejson._toggle_speedups(True)
        return result


class TestMissingSpeedups(tests.TestCase):
    def runTest(self):
...
def main():
    runner = tests.TextTestRunner(verbosity=1 + sys.argv.count('-v'))
