...
from src.modules import audio, video
from src.scenario import signals
from src.scenario import classes
from src.scenario import functions
...
def start_playing_audio_media(flag, **kwargs):
    ...
    if "player" in kwargs["_fsm"].vars.keys():
        try:
            ...
            log.error("CAN'T EXIT AUDIO PLAYER !")
            log.error(": " + str(e))
    kwargs["_fsm"].vars["player"] = video.OMXVideoPlayer(flag.args["args"][0],
                                                         hardware=False)
                            # audio.AlsaPlayer(flag.args["args"][0]) ou video.VLCPlayer(flag.args["args"][0])
    kwargs["_fsm"].vars["player"].start()
